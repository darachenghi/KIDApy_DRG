"""#SOLVES INTERVALS NO REDUCING
   ...
   Results are saved in folder {cluster_number}_results
      subfolders: reduced_networks, reduced_solutions, errors
...
eps = [1e-4, 2e-4, 5e-4,
       ...
       1e-1, 1.5e-1, 2e-1, 3e-1]   # 16 points

YEAR = 3600 * 24 * 365.25
ATOL = 1e-20
RTOL = 1e-3
CHUNK_SIZE = 20  #20 rows for each interval
COMPUTE_FULL_REF = False #solve the full (unreduced) network as an eps=0 reference for the error plots
...
def process_cluster(cluster_name):
    '''full pipeline for one cluster: solve (full ref + each eps, using already-reduced networks) -> errors -> plots -> summary'''
    global cluster
    ...
    for d in (SOLVED_DIR, ERROR_DIR, RED_NET_DIR):
        d.mkdir(parents=True, exist_ok=True)

    cluster = np.load(CLUSTER_PATH)  #set the module global so forked workers see it
    drg = DRG_p()
    drg.reduce_net(net, cluster, sources, eps, dropped, RED_NET_DIR)
    ...
    #per-eps L2 relative error and pointwise max relative error, each maxed over every interval AND every source species
    max_l2_rel_error_rows = [
        [e, max(max_rel_errors[s][i] for s in sources), max(max_max_errors[s][i] for s in sources)]
        for i, e in enumerate(eps)
    ]
    max_l2_rel_error_csv = SAVE_DIR / "max_l2_rel_error.csv"
    np.savetxt(max_l2_rel_error_csv, np.array(max_l2_rel_error_rows, dtype=np.float64), delimiter=",",
               header="epsilon,max_l2_rel_error,max_max_rel_error", comments="")
    print(f"Saved max L2 relative error to {max_l2_rel_error_csv}", flush=True)
